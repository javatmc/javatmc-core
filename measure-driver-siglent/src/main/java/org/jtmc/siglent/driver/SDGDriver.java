...
public class SDGDriver extends SCPISocketAdapter implements FunctionGenerator {
	...
	public SDGDriver(ISCPISocket adapter, DeviceIdentifier deviceIdentifier, SiglentFunctionGeneratorModel fGeneratorModel) {
	...
	private static class AnalogOutputImpl implements FunctionGenerator.AnalogOutput {
		...
		public AnalogOutputImpl(ISCPISocket socket, int channelId, long memoryDepth, int verticalResolution) {
			this.socket = socket;
		...
		public void setImpedance(double impedance) throws InstrumentException {
			try {
				...
				this.socket.send(channelCommand("OUTP").with("LOAD", impedance == Impedance.HIZ ? "HZ": String.valueOf(impedance)).build());
			} catch(IOException e) {
				throw new InstrumentException(e);
			}
		}
		...
		public void setWaveformType(WaveformType waveformType) throws InstrumentException{
			try {
				...
				this.socket.send(channelCommand("OUTP").with("WVTP", String.valueOf(waveformType.toString())).build());
			} catch(IOException e) {
				throw new InstrumentException(e);
			}
		}
