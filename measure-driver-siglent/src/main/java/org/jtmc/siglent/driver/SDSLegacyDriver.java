...
/**
 * Driver for the Siglent Oscilloscopes.
 * 
 * <p>Supported models are:
 * <ul>
 * <li>
 * SDS1000CFL
 * <li>
...
public class SDSLegacyDriver extends SCPISocketAdapter implements Oscilloscope {
  ...
  private static final int HORIZONTAL_DIVISIONS = 14;

  private static final int VERTICAL_DIVISIONS = 8;

  private static final double[] TIME_DIV_VALUES = new double[] {
      Units.nano(1), Units.nano(2), Units.nano(5), Units.nano(10), Units.nano(20),
      Units.nano(50), Units.nano(100), Units.nano(200), Units.nano(500),

      Units.micro(1), Units.micro(2), Units.micro(5), Units.micro(10), Units.micro(20),
      Units.micro(50), Units.micro(100), Units.micro(200), Units.micro(500),
      
      Units.milli(1), Units.milli(2), Units.milli(5), Units.milli(10), Units.milli(20),
      Units.milli(50), Units.milli(100), Units.milli(200), Units.milli(500), 1f,
  ...
  /**
   * Creates a new driver object for Siglent oscilloscopes.
   * 
   * @param socket SCPI Socket
   * @param deviceIdentifier Device identifier
   * @param info Siglent Oscilloscope model information
   * @throws IOException if there was an error communicating with the target
   */
  public SDSLegacyDriver(
      ISCPISocket socket,
      DeviceIdentifier deviceIdentifier,
      SiglentOscilloscopeModel info) throws IOException {
    ...
    this.acquisitionSystem = new AcquisitionSystemImpl(socket, 
  ...
  private static class ChannelImpl implements AnalogInput {
    ...
    public void setRange(double range) throws InstrumentException {
      try {
        double gain = range / verticalDivisions;
    ...
    public void setProbeAttenuation(double attenuation) throws InstrumentException {
      try {
        String attn = attenuation < 1
                        ? String.format("%.01f", attenuation) : String.format("%.00f", attenuation);
  ...
  private static class AcquisitionSystemImpl implements AcquisitionBaseSystem {
    ...
    public AcquisitionSystemImpl(
          ISCPISocket socket,
          int horizontalDivisions,
          double[] timeDivisionValues) {
      this.socket = socket;
      this.horizontalDivisions = horizontalDivisions;
      this.timeDivisionValues = timeDivisionValues;
    }

    private double getHorizontalScale(double secPerDiv) {
      return Arrays.stream(timeDivisionValues)
                   .filter(tdiv -> Math.abs(secPerDiv - tdiv) <= Units.PICO || tdiv >= secPerDiv)
                   .findFirst()
                   .getAsDouble();
    }
    ...
    public void setTimespan(double span) throws InstrumentException {
      try {
        ...
        double targetDiv = getHorizontalScale(secPerDiv);
