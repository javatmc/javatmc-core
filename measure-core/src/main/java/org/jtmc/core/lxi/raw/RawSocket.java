...
public class RawSocket implements ISocket {
  ...
  /**
   * Creates a new RawSocket using the IP address, port and board number.
   * 
   * @param host IP Address of the instrument
   * @param port Port number of the instrument
   * @param board Board number (not used)
   * @throws IOException if there was an error when connecting
   */
  ...
  public String toString() {
    return String.format("RawSocket[host=%s, port=%d, board=%d]",
                            host.getHostAddress(), port, board);
  }
  ...
  @Override
  public ByteBuffer receive(final int count, final long timeout)
  ...
  @Override
  public ByteBuffer receive(final char delimiter, final long timeout)
        throws IOException, SocketTimeoutException {
    int previousTimeout = socket.getSoTimeout();
    try {
      socket.setSoTimeout((int) timeout);
